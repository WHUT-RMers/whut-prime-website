...
.vp-link:hover:not(:disabled){ background:rgba(45,226,166,.08); color:var(--accent); border-color:var(--accent); }
/* 发送后倒计时：按钮变灰（灰字 + 灰边），倒计时结束自动恢复绿色 */
.vp-link.counting{
  color:var(--ink-faint); border-color:var(--line-strong); background:transparent;
  opacity:1;
}
